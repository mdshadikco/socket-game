...
io.on("connection", (socket) => {
    ...
    socket.on("ready-state-changed", ({ roomId, userId, isReady }) => {
        console.log(`✅ User ${userId} in room ${roomId} is now ${isReady ? "ready" : "not ready"}`);
    ...
    socket.on("join-room", ({ roomId, user, language }) => {
        ...
        const alreadyJoined = rooms[roomId].participants.some((u) => u.userId === user.userId);
        if (!alreadyJoined) {
            rooms[roomId].participants.push(Object.assign(Object.assign({}, user), { isReady: false }));
        }
        console.log(`🧑‍🤝‍🧑 ${user === null || user === void 0 ? void 0 : user.name} joined room ${roomId}`);
